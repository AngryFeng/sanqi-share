...
为了通用，Linux系统使用了三级页表（参考资料还停留在三级页表时代，实际X86-64架构已经是四级页表结构）结构：页目录、中间页目录和页表。PGD为顶级页表，是一个pgd_t数据类型（定义在文件linux/include/page.h中）的数组，每个数组元素指向一个中间页目录；PMD为二级页表，是一个pmd_t数据结构的数组，每个数组元素指向一个页表；PTE则是页表，是一个pte_t数据类型的数组，每个元素中含有物理地址。
...
```
| 9 bits (PGD) | 9 bits (PUD) | 9 bits (PMD) | 9 bits (PTE) | 12 bits (页内偏移) |
```

每一级页表大小：

- 每级页表为 **4KB 物理页**，包含 29=51229=512 个表项（每项8字节）。
- 每个表项存储下一级页表的物理地址或物理页框号（PFN）。

多级页表所有虚拟地址均被使用的`总页表内存=1+512+512*512+512*512*512=134,480,385≈513TB`(在全映射时比一级页表所占用空间更大，但此场景实际不存在)，实际的场景是稀疏映射：进程通常使用少量连续内存如1GB，对应极少数页表分配。

**示例对比：进程仅使用1GB内存**

**进程需求**：

- 虚拟地址范围：0x00000000 ~ 0x40000000（1GB）。
- 对应页数：1,GB/4,KB=262,1441 页。

**多级页表分配流程**：

1. **PGD表**：1个（4KB），管理所有虚拟地址。
   - 仅当虚拟地址的PGD索引对应区域被使用时，分配下级PUD表。
   - 本例中，1GB对应PGD索引范围较小（假设为前1个PGD项），分配1个PUD表。
2. **PUD表**：1个（4KB），管理512个PMD表。
   - 1GB对应1个PUD项，分配1个PMD表。
3. **PMD表**：1个（4KB），管理512个PTE表。
   - 1GB对应512个PMD项（但实际需要覆盖262,144页，需分配512个PTE表）。
4. **PTE表**：512个（每个4KB），管理262,144页。
   - 每个PTE表管理512页 → 512个PTE表共管理 512×512=262,144页。

**总页表内存**：

1,PGD+1,PUD+1,PMD+512,PTE=515,页表×4,KB=2.06,MB（注：实际计算中，PMD可能需要512个，总大小约4.1MB，但原理相同。）
...
在系统每次访问虚存页时，都要在内存的所有页表中寻找该页的页框，这是一个**很费时间**[^3]的工作。但是，人们发现，系统一旦访问了某一个页，那么系统就会在一段时间内稳定地工作在这个页上。所以，为了提高访问页表的速度，系统还配备了一组正好能容纳一个页表的硬件寄存器，这样当系统再访问虚存时，就首先到这组硬件寄存器中去访问，系统速度就快多了。这组存放当前页表的寄存器叫做**快表**。
...
[^3]:以 **x86-64架构的四级页表** 为例，虚拟地址到物理地址的转换需要 **4次内存访问**,每次内存访问需要 **数十到数百个CPU周期**，四级页表转换总耗时可能超过 **100个周期**，远高于CPU运算速度。