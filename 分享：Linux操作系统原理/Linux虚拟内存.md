...
**示例**：某进程使用1GB虚拟内存（2^30字节）：

- **页数**：`2^30/2^12=2^18=262,144`

  所需PTE表：`262,144/512=512` 个PTE表(每个PTE表管理512页)

  所需上级页表：

  - **PMD表**：512个（每个PMD表管理512个PTE表）
  - **PUD表**：1个（管理512个PMD表）
  - **PGD表**：1个

- **总页表内存**：
  `1(PGD)+1(PUD)+512(PMD)+512(PTE)=1,026 页表×4KB=4.1MB1(PGD)`

-  **对比一级页表**：512GB vs. **4.1MB**，内存占用减少 **125,000倍**。
